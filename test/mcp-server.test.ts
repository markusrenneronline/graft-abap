...
test('initialize carries instructions — the layer that survives tool deferral', async () => {
  ...
  assert.match(instructions, /select:.*exact tool names shown by your client/, 'uses the names actually exposed by the host');
  assert.match(instructions, /configured server prefix, not serverInfo.name/, 'does not confuse server identity with client registration');
  assert.doesNotMatch(instructions, /mcp__graft(?:-abap)?__/, 'does not hardcode either installation alias');
