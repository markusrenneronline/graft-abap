...
test("a failed sidecar write prevents publication of a slim graph", async () => {
  const dir = makeFixture();
  try {
    const outDir = contextDirFor(dir);
    const idxPath = askIndexPath(outDir);
    ...
    await assert.rejects(buildGraph(dir));
    assert.equal(readGraph(wiringPath(outDir)), null,
      "do not publish a graph whose method bodies have no searchable index");
  } finally {
    rmSync(dir, { recursive: true, force: true });
  }
});
