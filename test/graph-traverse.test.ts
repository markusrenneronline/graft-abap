...
test('resolveSymbol: a full path-scoped id is exact and never falls back to a same-named method', () => {
  const a = nodeStub({ id: 'src/a.abap#LCL.RUN', name: 'RUN', kind: 'method' });
  const b = nodeStub({ id: 'src/b.abap#LCL.RUN', name: 'RUN', kind: 'method' });
  const duplicate = nodeStub({ id: 'src/a.abap#LCL.RUN~2', name: 'RUN', kind: 'method' });
  const graph = graphOf([a, b, duplicate], []);
  assert.deepEqual(resolveSymbol(graph, 'SRC/A.ABAP#lcl.run'), [a]);
  assert.deepEqual(resolveSymbol(graph, 'src/a.abap#LCL.RUN~2'), [duplicate]);
  assert.deepEqual(resolveSymbol(graph, 'src/missing.abap#LCL.RUN'), []);
});
