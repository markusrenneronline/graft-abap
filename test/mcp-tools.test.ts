 ...
 * explicit depth 2 also reaches `compute`. Same fixture shape as
 * test/graph-traverse-cli.test.ts's `impact -d` test. */
function chainRepo(d = mkdtempSync(join(tmpdir(), 'graft-mcptools-chain-'))): string {
  mkdirSync(join(d, 'src'), { recursive: true });
  writeFileSync(
    join(d, 'src', 'math.ts'),
    'export function add(a: number, b: number): number {\n  return a + b;\n}\n' +
...
test('TOOLS lists the eight tools with schemas', async () => {
  ...
  assert.deepEqual((props.depth as { anyOf: unknown[] }).anyOf, [
...
test('graft_trace_calls: depth param is honored (depth 2 reaches further than depth 1)', async () => {
  ...
  assert.match(shallow.text, /^\[Trace\] effective depth: 1$/m);
  ...
  assert.match(deeper.text, /^\[Trace\] effective depth: 2$/m);
...
test('graft_trace_calls: numeric string depths equal numbers for incoming, outgoing and file traces', async () => {
  const d = chainRepo();
  for (const [symbol, direction] of [['add', 'in'], ['compute', 'out'], ['src/math.ts', 'out']]) {
    for (const depth of [1, 2, Number.MAX_SAFE_INTEGER]) {
      const numeric = await callTool(d, 'graft_trace_calls', { symbol, direction, depth, evidence: false });
      const args = { symbol, direction, depth: String(depth), evidence: false };
      const string = await callTool(d, 'graft_trace_calls', args);
      assert.equal(numeric.isError, false);
      assert.deepEqual(string, numeric, `${symbol} ${direction} depth ${depth}`);
      assert.equal(args.depth, String(depth), 'normalization must not mutate caller arguments');
    }
  }
});

test('graft_trace_calls: omitted depth is explicit 1; all and full expose the same unlimited walk', async () => {
  const d = chainRepo();
  const omitted = await callTool(d, 'graft_trace_calls', { symbol: 'add' });
  const one = await callTool(d, 'graft_trace_calls', { symbol: 'add', depth: 1 });
  assert.deepEqual(omitted, one);
  assert.match(omitted.text, /^\[Trace\] effective depth: 1$/m);
  assert.doesNotMatch(omitted.text, /← compute \(/);
  const all = await callTool(d, 'graft_trace_calls', { symbol: 'add', depth: 'all' });
  const full = await callTool(d, 'graft_trace_calls', { symbol: 'add', depth: 'full' });
  assert.equal(all.isError, false);
  assert.deepEqual(full, all);
  assert.match(all.text, /^\[Trace\] effective depth: all$/m);
  assert.match(all.text, /← compute \(/);
});

test('graft_trace_calls: invalid depths are errors before graph lookup, never rounded or defaulted', async () => {
  const bare = mkdtempSync(join(tmpdir(), 'graft-mcptools-invalid-depth-'));
  const invalid = [null, true, false, [], {}, 0, -1, 1.5, NaN, Infinity, -Infinity,
    Number.MAX_SAFE_INTEGER + 1, '', '0', '-2', '1.5', '2x', '02', '+2', ' 2', '2 ',
    '2\n', '1e2', '0x2', 'ALL', '9007199254740992'];
  for (const depth of invalid) {
    const result = await callTool(bare, 'graft_trace_calls', { symbol: 'add', depth });
    assert.equal(result.isError, true, String(depth));
    assert.match(result.text, /depth must be a positive safe integer/);
    assert.doesNotMatch(result.text, /effective depth:/);
  }
  const legacy = await callTool(bare, 'graft_callers', { symbol: 'add', depth: 'bad' });
  assert.match(legacy.text, /depth must be a positive safe integer/);
});

test('graft_trace_calls: workspace uses the same normalized depth and header as a repository', async () => {
  const parent = mkdtempSync(join(tmpdir(), 'graft-mcptools-workspace-depth-'));
  chainRepo(join(parent, 'repo'));
  writeWorkspace(parent, { version: 1, children: ['repo'] });
  for (const direction of ['in', 'out']) {
    const symbol = direction === 'in' ? 'add' : 'compute';
    const numeric = await callTool(parent, 'graft_trace_calls', { symbol, direction, depth: 2 });
    const string = await callTool(parent, 'graft_trace_calls', { symbol, direction, depth: '2' });
    assert.equal(numeric.isError, false);
    assert.deepEqual(string, numeric);
    assert.match(string.text, /^\[Trace\] effective depth: 2$/m);
    assert.match(string.text, /\[depth 2\]/);
    const all = await callTool(parent, 'graft_trace_calls', { symbol, direction, depth: 'all' });
    const full = await callTool(parent, 'graft_trace_calls', { symbol, direction, depth: 'full' });
    assert.equal(all.isError, false);
    assert.deepEqual(all, full);
    assert.match(all.text, /^\[Trace\] effective depth: all$/m);
    assert.match(all.text, /\[depth 2\]/);
  }
  const omitted = await callTool(parent, 'graft_trace_calls', { symbol: 'add' });
  assert.equal(omitted.isError, false);
  assert.match(omitted.text, /^\[Trace\] effective depth: 1$/m);
  assert.doesNotMatch(omitted.text, /← compute \(/);
  const invalid = await callTool(parent, 'graft_trace_calls', { symbol: 'add', depth: '2x' });
  assert.equal(invalid.isError, true);
  assert.match(invalid.text, /depth must be a positive safe integer/);
});
...
test('TOOLS advertises renamed tools plus the ABAP unresolved inventory', () => {
  assert.deepEqual([...TOOLS.map((t) => t.name)].sort(), [...Object.values(RENAMES), 'graft_unresolved_calls', 'graft_diagnostics'].sort());
});
