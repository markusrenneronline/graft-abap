 ...
 */
async function advertised(root: string, dirOverride?: string): Promise<typeof TOOLS> {
  if (dirOverride !== undefined) return TOOLS;
  const [{ hasGraftIndex }, { mainWorktreeRoot }] = await Promise.all([
    import('../graph/root.js'), import('../graph/seed.js'),
  ]);
 ...
 */
export async function startMcpServer(root: string, dirOverride?: string, version = '0'): Promise<void> {
  ...
  // A pinned integration explicitly disables upkeep. Do not load its host
  // installers (and their analysis dependencies) merely to return no lines.
  const upkeep = process.env.GRAFT_NO_UPKEEP === '1' ? []
    : (await import('../upkeep-run.js')).runUpkeep(root, runningVersion()).lines;
  ...
  // Concurrent first queries share the import. Initialization, ping and schema
  // discovery for an explicitly configured graph need no analysis engine.
  let runtime: Promise<typeof import('./tools.js')> | undefined;
  const toolRuntime = () => runtime ??= import('./tools.js');
  ...
  rl.on('line', (line) => {
    ...
    switch (method) {
      ...
      case 'tools/list': {
        ...
        advertised(root, dirOverride).then(
          tools => reply(id, { tools: tools.map((t) => ({ name: t.name, description: t.description, inputSchema: t.inputSchema })) }),
      ...
      case 'tools/call': {
        ...
        toolRuntime().then(({ callTool }) => callTool(root, name, args, dirOverride)).then(
          (r) => {
