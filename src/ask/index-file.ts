...
export interface AskIndex {
  version: 1;
  /** Identity of the indexed graph content; absent in legacy, untrusted caches. */
  graphKey?: string;
  avgBodyLen: number;
  df: [string, number][];
  docCount: number;
...
/** All graph fields used by lexical ranking, plus the body hash that survives
 * slim serialization. IDs/counts alone cannot distinguish edited method bodies. */
export function askIndexGraphKey(graph: GraphV1): string {
  const rows = graph.nodes.map(n => [n.id, n.name, n.path, n.body_hash, n.signature ?? '', n.summary ?? ''])
    .sort((a, b) => a[0].localeCompare(b[0]));
  return createHash('sha256').update(JSON.stringify(rows)).digest('hex');
}

export function askIndexMatches(graph: GraphV1, index: AskIndex | null): index is AskIndex {
  if (!index || index.graphKey !== askIndexGraphKey(graph) || index.docs.length !== graph.nodes.length) return false;
  const ids = new Set(index.docs.map(d => d.id));
  return ids.size === index.docs.length && graph.nodes.every(n => ids.has(n.id));
}
...
export function writeAskIndex(outDir: string, graph: GraphV1): string {
  ...
  const index: AskIndex = {
    version: 1,
    graphKey: askIndexGraphKey(graph),
    avgBodyLen,
    df: pairs(df),
...
export function readAskIndex(outDir: string): AskIndex | null {
  ...
  try {
    ...
    if (
      ...
      !Number.isFinite(raw.avgBodyLen) || raw.avgBodyLen < 0 ||
      (raw.graphKey !== undefined && (typeof raw.graphKey !== 'string' || !/^[a-f0-9]{64}$/.test(raw.graphKey))) ||
      !validBag(raw.df) ||
      raw.docs.some((d: unknown) => !validDoc(d)) ||
      new Set(raw.docs.map((d: AskIndexDoc) => d.id)).size !== raw.docs.length
    ) {
      return null;
    }
...
function validBag(value: unknown): boolean {
  ...
  return value.every(pair => {
    if (!Array.isArray(pair) || pair.length !== 2 || typeof pair[0] !== 'string' || !pair[0]
      || !Number.isSafeInteger(pair[1]) || pair[1] <= 0 || seen.has(pair[0])) return false;
    seen.add(pair[0]); return true;
  });
}

function validDoc(value: unknown): value is AskIndexDoc {
  if (!value || typeof value !== 'object') return false;
  const d = value as AskIndexDoc;
  return typeof d.id === 'string' && d.id.length > 0 && validBag(d.name) && validBag(d.path) && validBag(d.body);
}
