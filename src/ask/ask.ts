...
import { askIndexMatches, counts, tokenize, type AskIndex, type AskIndexDoc } from "./index-file.js";
...
function lexical(
  ...
  // A sidecar must cover every node AND match the indexed content identity.
  // Equal IDs cannot distinguish two versions of a method body. Missing/stale
  // caches use fields retained in the graph; slim graphs explicitly warn when
  // body search is incomplete. Normal pre-query refresh repairs the index.
  //
  ...
  if (graph && askIndexMatches(graph, corpus.askIndex)) {
...
export function ask(dir: string, query: string, opts: AskOptions = {}): AskResult {
  ...
  if (result.mode !== 'structural' && corpus.graph?.meta?.searchIndexKey
    && !askIndexMatches(corpus.graph, corpus.askIndex)
    && corpus.graph.nodes.some(n => n.kind !== 'file' && n.body_text === undefined)) {
    const warning = 'Search index missing, invalid or from different source content; method-body search is incomplete. Run graft build to repair it.';
    result.note = [result.note, warning].filter(Boolean).join('\n');
  }
  if (opts.source) {
...
function askSavingsLine(r: AskResult, body: string): string {
  if (process.env.GRAFT_NO_SAVINGS === "1") return "";
