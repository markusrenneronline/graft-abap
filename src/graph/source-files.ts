...
export function supportedExtensions(): string[] {
  return [...new Set([...depthExtensions(), ...genericExtensions(), ...containerExtensions(), ".abap"])].sort();
}
...
export function listSourceFiles(
  ...
  return filterByOnlyDirs(
    repoFiles.filter(
      (f) =>
        !f.startsWith(outDir) &&
        (isAbapFile(f) || languageOf(f) !== null || genericLangOf(f) !== null || containerLangOf(f) !== null),
    ),
    root,
    onlyDirs,
  );
