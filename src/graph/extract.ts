...
// Load only the grammar being used. An unavailable native grammar for an
// unrelated language must not prevent an ABAP-only repository from opening.
const requireGrammar = createRequire(import.meta.url);
const GRAMMARS: Record<Language, () => unknown> = {
  typescript: () => requireGrammar("tree-sitter-typescript").typescript,
  tsx: () => requireGrammar("tree-sitter-typescript").tsx,
  python: () => requireGrammar("tree-sitter-python"),
  go: () => requireGrammar("tree-sitter-go"),
  r: () => requireGrammar("tree-sitter-r"),
  java: () => requireGrammar("tree-sitter-java"),
  kotlin: () => requireGrammar("tree-sitter-kotlin"),
  swift: () => requireGrammar("tree-sitter-swift"),
  php: () => requireGrammar("tree-sitter-php").php,
};

let kotlinWasm: import('web-tree-sitter').Parser | undefined;
let kotlinWarm: Promise<void> | undefined;
/** Kotlin ships no native Windows prebuild. Reuse the bundled WASM grammar
 * with the same depth extractor rather than silently omitting Kotlin files. */
export async function warmDepthGrammars(languages: Iterable<Language>): Promise<void> {
  if (![...languages].includes('kotlin')) return;
  if (!kotlinWarm) kotlinWarm = (async () => {
    try { new Parser().setLanguage(GRAMMARS.kotlin() as never); return; }
    catch { /* no usable native binding: the bundled WASM is platform independent */ }
    const wasm = await import('web-tree-sitter');
    await wasm.Parser.init();
    const language = await wasm.Language.load(readFileSync(requireGrammar.resolve('tree-sitter-wasm/kotlin/tree-sitter-kotlin.wasm')));
    kotlinWasm = new wasm.Parser();
    kotlinWasm.setLanguage(language);
  })().catch(error => { kotlinWarm = undefined; throw error; });
  await kotlinWarm;
}
...
export function extractFile(rel: string, source: string, lang: Language): ExtractResult {
  if (lang === 'kotlin' && kotlinWasm) {
    const tree = kotlinWasm.parse(index => source.slice(index, index + PARSE_CHUNK));
    if (!tree) throw new Error('Kotlin WASM parser produced no syntax tree');
    try { return extractTree(rel, source, lang, tree.rootNode as unknown as Parser.SyntaxNode); }
    finally { tree.delete(); }
  }
  parser.setLanguage(GRAMMARS[lang]() as never);
  const root = parseSource(source);
  return extractTree(rel, source, lang, root);
}

function extractTree(rel: string, source: string, lang: Language, root: Parser.SyntaxNode): ExtractResult {
