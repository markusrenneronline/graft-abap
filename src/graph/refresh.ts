...
import { invalidateGraphCaches, loadGraphCached, loadAskIndexCached } from "./load.js";
import { askIndexMatches } from "../ask/index-file.js";
import { seedGraph, type SeedResult } from "./seed.js";
import { wiringPath } from "./write.js";
import { verifySourcePolicy } from "./source-snapshot.js";
...
function searchIndexCurrent(outDir: string): boolean {
  const graph = loadGraphCached(outDir);
  return !!graph && askIndexMatches(graph, loadAskIndexCached(outDir));
}
...
export async function ensureFreshGraph(root: string, opts: RefreshOptions = {}): Promise<RefreshResult> {
  ...
  try {
    ...
    // A completed generation is required even if source hashes match the old
    // graph: a producer may have begun replacing it without changing bytes yet.
    verifySourcePolicy(dir, outDir, readFingerprint(outDir)?.onlyDirs);
    ...
    if (drift && isClean(drift) && searchIndexCurrent(outDir)) return seedNote ? { refreshed: false, note: seedNote } : CLEAN;
    ...
    try {
      ...
      if (now && isClean(now) && searchIndexCurrent(outDir)) {
        verifySourcePolicy(dir, outDir, readFingerprint(outDir)?.onlyDirs);
      ...
      const built = await buildGraph(dir, { contextDir: opts.contextDir, graphOnly: true, onlyDirs });
      invalidateGraphCaches(outDir);
      const warnings = built.errors.length ? `graph built with output warnings: ${built.errors.join('; ')}` : undefined;
      return { refreshed: true, drift: drift ?? undefined, note: [seedNote, warnings].filter(Boolean).join('; ') || undefined };
    } finally {
      unhook();
      releaseLockIn(lockCache);
    }
    ...
    return { refreshed: false, note: `graph refresh skipped; answering from the last available graph (not verified current): ${err instanceof Error ? err.message : String(err)}` };
  }
}
