 ...
 *
 * A full path-scoped id containing # is matched exactly (case insensitive),
 * including any disambiguation ordinal; it never falls back to another file.
 * Otherwise matching, in order:
...
export function resolveSymbol(graph: GraphV1, query: string, opts: ResolveSymbolOptions = {}): NodeV1[] {
  // ABAP users naturally qualify members with => or ->. Internally the graph
  // uses the same dotted owner/member names as its other language adapters.
  const qualifiedAbap = !!graph.meta?.languages?.includes('abap')
    && (/=>|->/.test(query) || (/^[\w/]+\.[\w/~]+$/.test(query) && !isAbapFile(query)));
  if (graph.meta?.languages?.includes("abap")) query = query.replace(/=>|->/g, ".");
  const lowerQuery = query.toLowerCase();
  const looksLikeFilename = query.includes(".") && !query.includes("#");

  const exactId = query.includes('#');
  let matches = exactId ? graph.nodes.filter(node => node.id.toLowerCase() === lowerQuery) : symbolMatches(graph.nodes, lowerQuery);

  if (!exactId && !qualifiedAbap && matches.length === 0 && query.includes(".")) {
