...
import { extractFile, languageLabelOf, languageOf, warmDepthGrammars, type RawEdge, type Language } from "./extract.js";
...
import type { AbapExtraction } from "./abap.js";
import type { AbapParseStats } from "./abap-parse-cache.js";
import { isAbapFile } from "./abap-files.js";
import { SourceSnapshot, sourceDirectories } from "./source-snapshot.js";
import { contentHash } from "../util/id.js";
import { relPosix } from "../util/paths.js";
import { readSourceFile } from "../util/source.js";
import { readFollowNestedRepos, readFollowSubmodules, readIncludeDirs, writeJsonAtomic } from "../util/state.js";
import {
  emptyExtractCache,
  readExtractCache,
  writeExtractCache,
...
import { GRAPH_DIR, readGraph, writeGraph, wiringPath } from "./write.js";
import { writeCards, writeIndex, writeCovers, type CardStats } from "./cards.js";
import { askIndexGraphKey, writeAskIndex } from "../ask/index-file.js";
...
export interface GraphBuildResult {
  ...
  meaning: EnrichStats;
  errors: string[];
  /** ABAP analysis warnings (including unavailable SAP dependencies) do not make
   * a successful structural build fail. Full details live in the sidecar. */
  abap?: {
    files: number;
    diagnostics: number;
    byKind: Record<string, number>;
    diagnosticsPath: string;
    parser?: AbapParseStats;
  };
}
...
export async function buildGraph(
  ...
  opts = { ...opts, onlyDirs: sourceDirectories(outDir, opts.onlyDirs) };
  const snapshot = new SourceSnapshot(root, outDir, opts.onlyDirs);
  ...
  let abapParserStats: AbapParseStats | undefined;
  let unresolvedCalls: AbapExtraction["unresolvedCalls"] = [];
  let abapDiagnostics: AbapExtraction["diagnostics"] = [];
  ...
  let parsed = 0;
  let reused = 0;
  await warmDepthGrammars(files.map(f => languageOf(f.abs)).filter((lang): lang is Language => lang !== null));
  ...
  files.forEach((f, i) => {
    ...
    if (source === null) {
      if (isAbapFile(rel)) errors.push(`${rel}: unsupported source encoding; ABAP analysis cannot be complete`);
    ...
    if (isAbapFile(rel)) {
      // Resolved relationships are global. Object-level ASTs may be reused by
      // abaplint below, but never replay stale file-local semantic edges.
  ...
  if (abapSources.size > 0) {
    try {
      // Fresh graph queries and non-ABAP builds need no ABAP parser. Keep its
      // deferred import inside the failure boundary and snapshot validation.
      const [{ extractAbapFiles }, { abapParseSession }] = await Promise.all([
        import("./abap.js"), import("./abap-parse-cache.js"),
      ]);
      const session = abapParseSession(`${root}\0${outDir}`, opts.reuse !== false);
      const abap = extractAbapFiles(abapSources, session);
      abapParserStats = { ...session.stats };
      parsed -= abapParserStats.reusedFiles;
      reused += abapParserStats.reusedFiles;
      nodes.push(...abap.nodes);
      abapEdges = abap.edges;
      abapDiagnostics = abap.diagnostics;
      unresolvedCalls = abap.unresolvedCalls;
      // Retain source stats and hashes for freshness. These nodes are useful for
      // inspecting the cache, but the ABAP branch above deliberately never
      // replays them independently of the rest of the project.
      for (const node of abap.nodes) entries[node.path]?.nodes.push(node);
    } catch (err) {
      errors.push(`ABAP analysis failed — ${err instanceof Error ? err.message : String(err)}`);
    }
  }

  // Never replace a valid ABAP graph with a partial analysis. Unresolved SAP
  // dependencies are coverage notices; unreadable sources and parser failures
  // are build failures. The old graph and its fingerprint remain authoritative.
  const fatalAbap = abapDiagnostics.filter(d => d.kind === "parse_error" || d.kind === "metadata_error");
  if (fatalAbap.length || errors.some(e => e.startsWith("ABAP analysis failed") || /\.abap[: ]|\.xml[: ]/i.test(e))) {
    throw new Error(`ABAP analysis rejected; last valid graph retained: ${[...errors, ...fatalAbap.map(d => `${d.path}: ${d.message}`)].join("; ")}`);
  }
  ...
  // ABAP resolves with its own project registry. Do not offer its symbols to the
  // other languages' unqualified fallback resolver in a mixed-language repo.
  const edges = [
    ...resolveEdges(nodes.filter((node) => !isAbapFile(node.path)), rawEdges, {
  ...
  const graph: GraphV1 = {
    meta: {
      ...
      scopes,
      ...(opts.onlyDirs?.length ? { indexedDirectories: opts.onlyDirs } : {}),
    },
    nodes,
    edges,
    ...(abapSources.size ? { unresolvedCalls, abap: { files: abapSources.size, diagnostics: abapDiagnostics } } : {}),
  };
  ...
  // Use the same publication contract for the final graph and partial deep
  // checkpoints: slim nodes must never be published without their body index.
  const publishGraph = (): string => {
    snapshot.assertStable();
    graph.meta.searchIndexKey = askIndexGraphKey(graph);
    writeAskIndex(outDir, graph);
    snapshot.assertStable();
    return writeGraph(graph, outDir);
  };
  ...
  const graphPath = publishGraph();
  const abapDiagnosticsPath = join(outDir, GRAPH_DIR, "abap-diagnostics.json");
  if (abapSources.size > 0 || existsSync(abapDiagnosticsPath)) {
    try {
      // Rewrite on every build, including deletion of the last ABAP file, so
      // diagnostics never outlive the source set that produced them.
      writeJsonAtomic(abapDiagnosticsPath, {
        version: 1,
        files: abapSources.size,
        diagnostics: abapDiagnostics,
      });
    } catch (err) {
      errors.push(`abap-diagnostics: ${err instanceof Error ? err.message : String(err)}`);
    }
  }
  ...
  const abapByKind: Record<string, number> = {};
  for (const diagnostic of abapDiagnostics) {
    abapByKind[diagnostic.kind] = (abapByKind[diagnostic.kind] ?? 0) + 1;
  }
  ...
  return {
    ...
    ...(abapSources.size > 0 ? {
      abap: {
        files: abapSources.size,
        diagnostics: abapDiagnostics.length,
        byKind: abapByKind,
        diagnosticsPath: abapDiagnosticsPath,
        parser: abapParserStats,
      },
    } : {}),
  };
}
