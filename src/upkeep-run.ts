...
export function runUpkeep(
  ...
  // An isolated/pinned integration must not rewrite host configuration or
  // replace its adapter with a newer upstream build during a read request.
  if (process.env.GRAFT_NO_UPKEEP === '1') return { lines: [] };
  const lines: string[] = [];
  try {
